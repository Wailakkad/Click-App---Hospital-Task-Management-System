...
export default function NurseTasksPage() {
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center py-16">
      ...
      <p className="text-slate-500 text-center max-w-md">You're all caught up! New patient care tasks will appear here when they're assigned to you.</p>
    </div>
  );
      ...
      <aside className="w-72 bg-white border-r border-slate-200 shadow-sm">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-lg font-semibold text-slate-900 truncate">
                Nurse {user?.name || 'Professional'}
              </h2>
              <p className="text-sm text-slate-500 truncate">{user?.email || 'nurse@hospital.com'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-700">CareHub</h3>
              <p className="text-xs text-slate-500">Nursing Dashboard</p>
            </div>
          </div>
        </div>
        ...
        <nav className="p-6">
          <ul className="space-y-2">
            <li>
              <a className="flex items-center space-x-3 text-slate-600 hover:text-purple-600 hover:bg-purple-50 px-3 py-2 rounded-lg transition-colors cursor-pointer">
            ...
            <li>
              <a className="flex items-center space-x-3 text-purple-600 bg-purple-50 border border-purple-200 px-3 py-2 rounded-lg font-medium">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center space-x-3 mb-2">
                <h1 className="text-2xl font-bold text-slate-900">My Care Tasks</h1>
                <div className="px-2 py-1 bg-purple-100 text-purple-700 rounded-md text-xs font-medium">
                  {user?.role?.toUpperCase() || 'NURSE'}
                </div>
              </div>
              <p className="text-slate-600">
                Welcome back, Nurse {user?.name || 'Professional'}! Manage your patient care tasks and responsibilities
              </p>
            </div>
              ...
              {tasks.map((task) => (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <h3 className="font-semibold text-lg text-slate-900 group-hover:text-purple-600 transition-colors line-clamp-2">
                      {task.title}
                    </h3>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="w-2 h-2 rounded-full bg-slate-300 group-hover:bg-purple-400 transition-colors"></div>
                  </div>
                </div>
              ))}
            </div>
